...
Right-clicking an image and choosing **Open in Photoshop** is the core action, and it works on `LoadImage`-style nodes, generated previews, and saved outputs. On top of that, the pack adds nine nodes (category `image/photoshop`):
...
The round trip, the nine nodes, the gallery, and cross-machine editing all work today. Still on the roadmap:
