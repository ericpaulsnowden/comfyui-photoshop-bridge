...
- **`.ai` and raw/`.dng` loading need an optional decoder.** `.tif`/`.tiff` works out of the box. For Illustrator `.ai`, `pip install pypdfium2`; for `.dng` and camera raw, `pip install rawpy` (both are lightweight prebuilt wheels, no system dependencies). Until they're installed, those formats simply don't appear in the Load PSD file list. `.ai` renders its embedded preview; raw is a standard demosaic, not a color-managed develop.
