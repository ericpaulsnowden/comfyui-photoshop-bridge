...
Right-clicking an image and choosing **Open in Photoshop** is the core action, and it works on `LoadImage`-style nodes, generated previews, and saved outputs. On top of that, the pack adds six nodes (category `image/photoshop`):
...
- **Photoshop Live Canvas** — realtime drawing: toggle **Live Mode** in the plugin panel and this node serves a fresh snapshot of the canvas you're drawing on after **every stroke — no saving** (a lightweight change-detect poll, downscaled capture, ~sub-second). With the node's **`auto_queue`** widget On, every stroke queues a re-render automatically (coalesced — always your newest strokes, never a backlog). A ready-made workflow ships in [`examples/live_drawing_lcm.json`](examples/live_drawing_lcm.json): drop it in, point the checkpoint at an **LCM- or Lightning-class model** (1–4 steps, CFG 1–2, denoise ~0.5 — the denoise is your "AI creativity" knob; the seed is fixed so only your drawing changes the output), start Live Mode, draw. Frames are ephemeral — never saved to disk, never in the gallery. **Requires the Tier-2 plugin** (save-free capture is impossible without it). MASK output is always empty (the live stream is JPEG); derive masks downstream.
...
The round trip, the six nodes, the gallery, and cross-machine editing all work today. Still on the roadmap:
