...
**Prompt control (shipped in M2, added after Eric's first live test):** a **Photoshop Live Prompt** node (STRING out) fed from a **LIVE PROMPT** field in the panel — steer the render from inside Photoshop, no tabbing to the graph. Streams as `live_prompt` (keep-latest slot), re-renders via the same `requestQueue()` seam as frames (`cpsb.liveprompt`). NOT Tier-2-gated: falls back to its own widget so ComfyUI-only still works. Bundled workflow tuned for prompt influence: **CFG 2.0, denoise 0.65** (the "prompt doesn't change anything" symptom at denoise 0.5 was the drawing's latent dominating + weak low-CFG LCM adherence + the `lcm` sampler on a non-few-step checkpoint — an on-canvas Note now states the model requirement and the denoise-as-creativity relationship).
